...
chmod +x "$START_SCRIPT_DST" "$TRACK_SCRIPT_DST"
...
start_payload="$(jq -n --arg sid "$test_session_id" --arg tp "$test_transcript" '{session_id: $sid, source: "selftest", transcript_path: $tp, cwd: "/selftest"}')"
